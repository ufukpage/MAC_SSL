...
class JpegItem(object):

    def __init__(self, video_info: dict, data_dir: str, frame_fmt: str):
        self.data_dir = os.path.join(data_dir, video_info['name'].split("/")[1].split('.')[0])
...
class JpegBackend(object):
    ...
    def open(self, video_info) -> JpegItem:
        return JpegItem(video_info, self.data_dir, self.frame_fmt)
