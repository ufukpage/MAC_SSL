...
class RandomFrameSampler(object):
    ...
    def sample_single_bbox(self, sf: int, ef: int):
        num_frames = ef-sf + 1
        stride = random.choice(self.strides)
        if stride > 1 and self.temporal_jitter:
            index_jitter = [random.randint(0, stride-1) for i in range(self.clip_len)]
        ...
        if total_len >= num_frames:
            ...
            start_index = random.randint(sf, ef - total_len)

        frame_inds = [min(start_index + i * stride + index_jitter[i], ef)
                      for i in range(self.clip_len)]
        return frame_inds
    ...
    def sample_bbox(self, sf: int, ef: int):
        return np.array([self.sample_single_bbox(sf, ef) for _ in range(self.num_clips)], np.int)
