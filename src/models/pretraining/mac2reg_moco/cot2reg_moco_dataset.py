...
class CoT2RegMocoDataset(TSNDataset):

    def __init__(self, *args, **kwargs):
        super(CoT2RegMocoDataset, self).__init__(*args, **kwargs)
        ...
        try:
            self.mix_alpha_trans_index = \
                next(i for i, trans in enumerate(self.img_transform.transforms)
                     if trans.__class__.__name__ == 'GroupMask2Calculation')
        except Exception:
            logger = logging.getLogger()
            logger.error("Cannot find 'GroupMask2Calculation' in "
                         "the image transformation configuration."
                         "It is necessary for CoT2RegMoco task.")
            raise ValueError
    ...
    def __getitem__(self, idx):
        ...
        if not self.test_mode:
            ...
            data['imgs_k'] = DataContainer(imgs_k,
                                           ...
                                           pad_dims=None,
                                           cpu_only=False)
            gt_label = torch.cat([torch.FloatTensor([alpha1]),  torch.FloatTensor([alpha2])])
