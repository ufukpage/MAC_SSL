...
class EvalHook(Hook):
    ...
    def before_train_epoch(self, runner):
        """Evaluate the pt_model_3 only at the start of training."""
...
class SSLEvalHook(EvalHook):
    def __init__(self,
                 ...
                 tmpdir=None,
                 **eval_kwargs):
        super(SSLEvalHook, self).__init__(
            dataloader, start=start, interval=interval, **eval_kwargs)
        self.tmpdir = tmpdir

    def after_train_epoch(self, runner):
        from ...apis import single_ssl_gpu_test
        if not self.evaluation_flag(runner):
            return
        results, labels = single_ssl_gpu_test(runner.model, self.dataloader)
        self.evaluate_ssl(runner, results, labels)

    def evaluate_ssl(self, runner, results, labels):
        eval_res = self.dataloader.dataset.evaluate_ssl(
            results, np.array(labels), logger=runner.logger, **self.eval_kwargs)
        for name, val in eval_res.items():
            runner.log_buffer.output[name] = val
        runner.log_buffer.ready = True


class SSLDistEvalHook(SSLEvalHook):
    def __init__(self,
                 dataloader,
                 start=None,
                 interval=1,
                 tmpdir=None,
                 **eval_kwargs):
        super(SSLDistEvalHook, self).__init__(
            dataloader, start=start, interval=interval, **eval_kwargs)
        self.tmpdir = tmpdir

    def after_train_epoch(self, runner):
        from ...apis import multi_ssl_gpu_test
        if not self.evaluation_flag(runner):
            return
        tmpdir = self.tmpdir
        if tmpdir is None:
            tmpdir = osp.join(runner.work_dir, 'ssl_eval_hook')
        gt_tmpdir = osp.join(runner.work_dir, 'ssl_gt_eval_hook')
        results, labels = multi_ssl_gpu_test(
            runner.model,
            self.dataloader,
            tmpdir=tmpdir, gt_tmpdir=gt_tmpdir)
        if runner.rank == 0:
            print('\n')
            self.evaluate_ssl(runner, results, labels)
