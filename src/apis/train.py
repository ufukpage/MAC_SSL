...
from ..core import EvalHook, DistEvalHook, SSLEvalHook, SSLDistEvalHook
...
def train_network(model,
    ...
    # run linear probe mode
    if cfg.get('linear_probe', False):
        for v in model.module.backbone.parameters():
    ...
    # register eval hooks
    if validate:
        ssl_mode = cfg.get("ssl_mode", False)
        ...
        if ssl_mode:
            eval_hook = SSLDistEvalHook if distributed else SSLEvalHook
        else:
